"""
Tool-Orchestrator Gateway

LLM-driven pipeline: Athena (LLM) chooses which skills to call via tool use.
Primary orchestration path — generate_answer.py is the fallback.
"""
...
_WEIGHTS_FILE = Path(__file__).parent.parent.parent.parent.parent / "data" / "brain" / "training_weights.json"
...
def _get_training_guidance() -> str:
    ...
    if not _WEIGHTS_FILE.exists():
        return ""
    try:
        weights = json.loads(_WEIGHTS_FILE.read_text())
        weak = [cat for cat, w in weights.items() if isinstance(w, dict) and w.get("weight", 1.0) < 0.6]
        if not weak:
            return ""
        return (
            f"\nCAUTION: You have historically been weak on: {', '.join(weak)}. "
            "Double-check any claims in these areas against the knowledge base.\n"
        )
    except Exception:
        return ""
...
async def process_with_tools(
    ...
    system = f"""You are Athena, the Chief of Staff for {cfg.company.agent_name} ({cfg.persona.role}) at {cfg.company.name}.
...
Example: User says "Give me 10 customer names"
  ...
  RIGHT: Call memory_search("{cfg.company.name} customers") + customer_lookup("customers") + research_skill("customer list") simultaneously, then compile results.
...
YOUR TOOLS
═══════════════════════════════════════════════════
- memory_search: Search Mem0 long-term memory. Try MULTIPLE user_ids: "{cfg.memory.customers}", "{cfg.memory.knowledge}", "{cfg.memory.general}". Try DIFFERENT search terms.
...
- lead_intelligence: Get intelligence on a company/lead — news, expansions, industry trends. Use for outreach prep.
...
For internal data (customers, orders, history):
  1. memory_search with user_id="{cfg.memory.customers}"
  2. memory_search with user_id="{cfg.memory.knowledge}"
  3. customer_lookup
  4. research_skill
...
For external data (companies, market, competitors):
  1. web_search
  2. research_skill
  3. memory_search with user_id="{cfg.memory.general}"
