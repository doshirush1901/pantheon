    ...
    def retrieve_for_prompt(
        ...
        if self._pg:
            try:
                ...
                logger.error("Caught exception in HybridEpisodicStore.retrieve_for_prompt (PostgreSQL): %s", e, exc_info=True)
        ...
        try:
            ...
            logger.error("Caught exception in HybridEpisodicStore.retrieve_for_prompt (Mem0): %s", e, exc_info=True)
    ...
    def get_recent(self, identity_id: str, limit: int = 10) -> list:
        ...
        if self._pg:
            try:
                ...
                logger.error("Caught exception in HybridEpisodicStore.get_recent (PostgreSQL): %s", e, exc_info=True)
        
        try:
            results.extend(self._mem0.get_recent(identity_id, limit))
        except Exception as e:
            logger.error("Caught exception in HybridEpisodicStore.get_recent (Mem0): %s", e, exc_info=True)
...
class HybridProceduralStore:
    ...
    def match_procedure(self, query: str):
        ...
        if self._pg:
            try:
                ...
                logger.error("Caught exception in HybridProceduralStore.match_procedure (PostgreSQL): %s", e, exc_info=True)
        
        # Fall back to Mem0
        try:
            return self._mem0.match_procedure(query)
        except Exception as e:
            logger.error("Caught exception in HybridProceduralStore.match_procedure (Mem0): %s", e, exc_info=True)
            return None
    ...
    def get_procedure_guidance(self, procedure) -> str:
        ...
        if hasattr(self._mem0, 'get_procedure_guidance'):
            try:
                ...
                logger.error("Caught exception in HybridProceduralStore.get_procedure_guidance (Mem0): %s", e, exc_info=True)
        ...
        if self._pg and hasattr(self._pg, 'get_procedure_guidance'):
            try:
                ...
                logger.error("Caught exception in HybridProceduralStore.get_procedure_guidance (PostgreSQL): %s", e, exc_info=True)
    ...
    def record_outcome(self, procedure_id: str, success: bool):
        ...
        try:
            ...
            logger.error("Caught exception in HybridProceduralStore.record_outcome (Mem0): %s", e, exc_info=True)
        ...
        if self._pg:
            try:
                ...
                logger.error("Caught exception in HybridProceduralStore.record_outcome (PostgreSQL): %s", e, exc_info=True)
    ...
    def get_all(self) -> list:
        ...
        if self._pg:
            try:
                ...
                logger.error("Caught exception in HybridProceduralStore.get_all (PostgreSQL): %s", e, exc_info=True)
        
        try:
            results.extend(self._mem0.get_all())
        except Exception as e:
            logger.error("Caught exception in HybridProceduralStore.get_all (Mem0): %s", e, exc_info=True)
