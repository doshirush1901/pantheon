...
# =============================================================================
# SIMPLIFIED UNIFIED INTERFACE (add_memory / get_memories / clear_memories)
# =============================================================================

def _get_mem0_for_unified():
    """Get Mem0 service for the simplified interface."""
...
class UnifiedMemoryBackend:
    """
    Simplified unified memory interface for add/get/clear operations.

    Use get_unified_memory_backend() to obtain the singleton instance.
    """

    async def add_memory(self, user_id: str, memory_text: str, memory_type: str = "fact") -> None:
        """Add a memory for the given user."""
        import asyncio
        mem0 = _get_mem0_for_unified()
        metadata = {"memory_type": memory_type} if memory_type else {}
        await asyncio.to_thread(mem0.add_memory, memory_text, user_id, metadata)

    async def get_memories(self, user_id: str, query: str, top_k: int = 10) -> List[str]:
        """Retrieve relevant memories for the user."""
        import asyncio
        mem0 = _get_mem0_for_unified()
        results = await asyncio.to_thread(mem0.search, query, user_id, limit=top_k)
    ...
    async def clear_memories(self, user_id: str) -> None:
        ...
        import asyncio
        mem0 = _get_mem0_for_unified()
        await asyncio.to_thread(mem0.delete_all, user_id)


_unified_memory_backend: Optional[UnifiedMemoryBackend] = None


def get_unified_memory_backend() -> UnifiedMemoryBackend:
    """Get the simplified unified memory backend (singleton)."""
    global _unified_memory_backend
    if _unified_memory_backend is None:
        _unified_memory_backend = UnifiedMemoryBackend()
    return _unified_memory_backend


# =============================================================================
# SINGLETON ACCESS (legacy UnifiedMemoryService)
# =============================================================================
