    ...
    def _store_json(self, update: MemoryUpdate, decision: ControllerDecision):
        ...
        if filepath.exists():
            try:
                ...
                logger.error(f"Error in _store_json: {e}", exc_info=True)
    ...
    def _queue_conflict(self, update: MemoryUpdate, decision: ControllerDecision):
        ...
        if conflict_file.exists():
            try:
                ...
                logger.error(f"Error in _queue_conflict: {e}", exc_info=True)
    ...
    def process(
        ...
        if identity_id:
            try:
                ...
                logger.debug(f"Unified mem0 not available for identity resolution: {e}")
