...
Merged memory (one sentence):"""
    ...
    def _table_exists(self, table_name: str, schema: str = "public") -> bool:
        """Check if a Postgres table exists, using a fresh cursor to avoid
        poisoning the caller's transaction on failure."""
        try:
            conn = self._get_db()
            if not conn:
                return False
            ...
            try:
                ...
                logger.error(f"Error in _table_exists: {e}", exc_info=True)
            return False

    def _mem_table_and_schema(self):
        """Return (schema-qualified table, schema, table) for whichever
        memory table actually exists in the database."""
        for schema, table in [
            ("ira_memory", "user_memories"),
            ("public", "persistent_memory"),
            ("public", "user_memories"),
        ]:
            if self._table_exists(table, schema):
                return f"{schema}.{table}", schema, table
    ...
    def decay_old_memories(self, days: int = 30) -> int:
        ...
        try:
            ...
            fq_table, _, _ = self._mem_table_and_schema()
            if not fq_table:
                print("[memory_intelligence] No memory table found in DB, skipping decay")
                return 0
            ...
            try:
                ...
                logger.error(f"Error in decay_old_memories: {e}", exc_info=True)
            return 0
    ...
    def archive_memories(self, days: int = 180) -> int:
        ...
        try:
            ...
            fq_table, _, _ = self._mem_table_and_schema()
            if not fq_table:
                print("[memory_intelligence] No memory table found in DB, skipping archive")
                return 0
            ...
            try:
                ...
                logger.error(f"Error in archive_memories: {e}", exc_info=True)
            return 0
