...
class ReasoningTrace:
    ...
    - used_pattern_ids: LearningHub pattern IDs that informed this trace
    - hedging_required: True when patterns have low calibrated confidence
    """
    inner_monologue: str = ""
    ...
    def to_prompt_context(self) -> str:
        ...
        if self.hedging_required:
            parts.append(
                "\n⚠️ CONFIDENCE NOTE: Some patterns I'm relying on have low "
                "historical accuracy. I should hedge my statements — use phrases "
                "like 'there is a chance that...', 'based on what I've seen so "
                "far...', 'this is a tentative assessment' instead of definitive "
                "claims."
            )
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "used_pattern_ids": self.used_pattern_ids,
            "hedging_required": self.hedging_required,
        }
...
class MemoryReasoner:
    ...
    Consults the LearningHub for relevant patterns and logs verifiable
    predictions to the PredictionLog.
    """

    _PREDICTION_SIGNALS = re.compile(
        r"(will likely|probably will|expect(s|ed)? to|should (close|churn|reply|convert|order))",
        re.IGNORECASE,
    )
    ...
    def _get_learning_hub(self):
        if self._learning_hub is None:
            try:
                from openclaw.agents.ira.src.learning.learning_hub import get_learning_hub
                self._learning_hub = get_learning_hub()
            except Exception as e:
                logger.debug("LearningHub not available: %s", e)
        return self._learning_hub

    def _get_prediction_log(self):
        if self._prediction_log is None:
            try:
                from openclaw.agents.ira.src.learning.prediction_log import get_prediction_log
                self._prediction_log = get_prediction_log()
            except Exception as e:
                logger.debug("PredictionLog not available: %s", e)
    ...
    def reason(
        ...
        Enriches the trace with learned patterns from the LearningHub,
        applies hedging when pattern confidence is low, and logs
        verifiable predictions to the PredictionLog.
        """
        ...
        if force_deep or self._needs_deep_reasoning(message, memory_count):
            logger.info("Using DEEP reasoning (%d memories)", memory_count)
            trace = self.deep.reason(message, user_memories, entity_memories, context)
        else:
            logger.info("Using FAST reasoning (%d memories)", memory_count)
            trace = self.fast.reason(message, user_memories, entity_memories, context)

        self._enrich_from_learning_hub(trace, message)
        self._maybe_log_prediction(trace, message, context)
    ...
    def _enrich_from_learning_hub(self, trace: ReasoningTrace, message: str):
        """
        Look up relevant learned patterns and fold them into the trace.

        If any matched pattern has effective_confidence < HEDGE_THRESHOLD,
        set hedging_required so the prompt instructs cautious language.
        """
        hub = self._get_learning_hub()
        if hub is None:
            return

        try:
            active = hub.get_active_patterns(min_confidence=0.1)
        except Exception as e:
        ...
        for p in active:
            pattern_words = set(p.content.lower().split())
            if not pattern_words:
                continue
            overlap = len(msg_words & pattern_words)
            if overlap >= 2 or (overlap >= 1 and len(msg_words) <= 4):
                matched.append(p)
        ...
        for p in matched[:5]:
            eff = p.effective_confidence()
            trace.used_pattern_ids.append(p.pattern_id)
            trace.relevant_facts.append(
                f"[learned, conf={eff:.0%}] {p.content}"
            )

            hub.reinforce_pattern(p.pattern_id)

            if eff < HEDGE_THRESHOLD:
                any_low_confidence = True

        if any_low_confidence:
            trace.hedging_required = True
            trace.action_hints.append(
                "Hedge predictions — some patterns have low historical accuracy"
            )
    ...
    def _maybe_log_prediction(
        ...
        """
        If the trace's action hints or inner monologue contain a verifiable
        prediction (deal close, churn, reply), log it to the PredictionLog.
        """
        pred_log = self._get_prediction_log()
        if pred_log is None or not trace.used_pattern_ids:
            return
        ...
        if not self._PREDICTION_SIGNALS.search(combined_text):
            return

        predicted_outcome = combined_text[:200]
        ctx = dict(context or {})
        ctx["message_excerpt"] = message[:100]
        ctx["trace_confidence"] = trace.confidence

        for pid in trace.used_pattern_ids[:3]:
            try:
                pred_log.record_prediction(
                    pattern_id=pid,
