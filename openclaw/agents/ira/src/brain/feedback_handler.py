...
def detect_feedback(message: str, previous_response: str = "") -> Tuple[Optional[str], float]:
    ...
    (e.g. "actually, can you tell me about PF2?" is NOT negative feedback).
    ...
    # High-confidence: clear majority of one type with no ambiguity
    if pos_hits >= 3 and neg_hits == 0:
        return "positive", 0.9
    if neg_hits >= 3 and pos_hits == 0:
        return "negative", 0.9

    # Ambiguous: both positive and negative patterns, or low hit count
    # Use LLM to disambiguate when a previous response is available
    if previous_response and (pos_hits > 0 and neg_hits > 0):
...
def _llm_classify_feedback(message: str, previous_response: str) -> Optional[str]:
    """Use LLM to classify ambiguous messages as feedback or new query."""
    try:
        import openai
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "POSITIVE_FEEDBACK - user is praising the previous response\n"
                    "NEGATIVE_FEEDBACK - user is correcting or criticizing the previous response\n"
                    "NEW_QUERY - user is asking a new question (not feedback)\n\n"
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "Example input: 'Company A is shut, they are not a customer anymore. Company B has PF1-X-1310'\n"
                    "Example output:\n"
                    "Company A is permanently closed and no longer operational\n"
                    "Company A is NOT a current customer of our company\n"
                    "Company B has machine PF1-X-1310\n"
                    "Company B is a confirmed customer of our company\n\n"
...
def _identify_agents_used(generation_path: str, agents_used: Optional[List[str]] = None) -> List[str]:
    ...
    Prefers the explicit agents_used list (set by tool_orchestrator) when available.
    Falls back to string matching on generation_path for legacy paths.
    """
    if agents_used:
