...
class IngestionResult:
    ...
    def __str__(self):
        ...
        excrete_info = f" (excreted {self.items_excreted} low-quality)" if self.items_excreted else ""
        skip_info += excrete_info
        return (
...
class KnowledgeIngestor:
    ...
    def ingest_batch(self, items: List[KnowledgeItem]) -> IngestionResult:
        ...
        if result.items_excreted:
            self._log(f"  🚽 Excreted {result.items_excreted} low-quality items (score < {MIN_QUALITY})")
