...
def _load_learned_hints() -> List[TruthHint]:
    """Load truth hints generated by Nemesis during sleep training."""
    hints_file = Path(__file__).parent / "learned_truth_hints.json"
    if not hints_file.exists():
        return []
    try:
        import json
        raw = json.loads(hints_file.read_text())
        loaded = []
        for h in raw:
            loaded.append(TruthHint(
                id=h.get("id", "learned_unknown"),
                question_patterns=h.get("question_patterns", []),
                answer=h.get("answer", ""),
                category=h.get("category", "learned"),
                confidence=0.90,
...
LEARNED_HINTS: List[TruthHint] = _load_learned_hints()
ALL_HINTS: List[TruthHint] = TRUTH_HINTS + LEARNED_HINTS
...
class TruthHintMatcher:
    """Matches queries to truth hints (including Nemesis-learned hints)."""

    def __init__(self, hints: List[TruthHint] = None):
        self.hints = hints or ALL_HINTS
