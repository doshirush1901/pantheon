...
_imports_fallback = None
_KNOWLEDGE_SOURCES_INITIALIZED = False

def _init_knowledge_sources():
    """Lazy-init connections to Qdrant, Mem0, Neo4j, and imports fallback."""
    global _qdrant_retrieve, _mem0_service, _neo4j_store, _imports_fallback, _KNOWLEDGE_SOURCES_INITIALIZED
    ...
    try:
        from openclaw.agents.ira.src.brain.imports_fallback_retriever import fallback_retrieve
        _imports_fallback = fallback_retrieve
        logger.info("Clio: Imports fallback retriever connected (via Alexandros archive)")
    except ImportError:
        try:
            from imports_fallback_retriever import fallback_retrieve
            _imports_fallback = fallback_retrieve
            logger.info("Clio: Imports fallback retriever connected (relative import)")
        except ImportError:
            logger.warning("Clio: Imports fallback retriever unavailable")
...
async def _parallel_search(
    ...
    Falls back to raw imports scanning if primary sources are sparse."""
    results = []
    ...
    substantive = [r for r in results if r.get("relevance", 0) >= 0.4
                   and r.get("type") != "general_info"]
    if len(substantive) < 3 and _imports_fallback is not None:
        logger.info("Clio: Primary sources sparse (%d results) — trying imports fallback", len(substantive))
        try:
            fallback_results = _imports_fallback(query)
            if fallback_results:
                results.extend(fallback_results)
                logger.info("Clio: Imports fallback added %d results", len(fallback_results))
        except Exception as e:
            logger.warning("Clio: Imports fallback failed: %s", e)
...
async def _search_graph(query: str, machines: List[str]) -> List[Dict]:
    ...
    entity_patterns = [
        ...
        (r'\b(Machinecraft|FRIMO|Sister-Company|OEM-Partner|Tata|OEM-Auto)\b', lambda m: m.group(0)),
    ]
