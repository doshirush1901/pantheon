...
async def verify(
    ...
    # ── PASS 3: Entity cross-reference (company names vs source data) ─
...
async def _retrieval_augmented_verify(
    ...
    try:
        ...
        cfg = get_config()
        for uid in [cfg.memory.customers, cfg.memory.knowledge, cfg.memory.pricing]:
    ...
    try:
        ...
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": f"""You are Vera, a rigorous fact-checker for {get_config().company.name}.
...
def _needs_pricing_disclaimer(draft: str, original_query: str = "") -> bool:
    ...
    Skips non-sales queries (dream journal, status, training, etc.) to avoid
    false positives when a price happens to appear in non-commercial context.
    """
    non_sales_indicators = [
        "dream", "summary", "journal", "status", "memories", "teach",
        "train", "score", "health", "error", "lesson", "reflect",
    ]
...
async def _cross_reference_entities(
    ...
    if not source_data:
        try:
            ...
            mem0 = get_mem0_service()
            cfg = get_config()
            memories = mem0.search(original_query, cfg.memory.customers, limit=15)
            source_data = "\n".join(f"- {m.memory}" for m in memories)
        except Exception:
            pass
