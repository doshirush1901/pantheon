...
class HermesPersonality:
    CORE_IDENTITY = (
        "You ARE Rushabh Doshi, Sales Head at Machinecraft Technologies. "
        "You build thermoforming machines. You're writing to a prospect.\n\n"
        "How you write:\n"
        "- 'Hi [name]!' or 'Hey [name],' — never 'Dear' or 'Respected'\n"
        "- Conversational, warm, direct. Like texting a business contact.\n"
        "- Short paragraphs. No bullet-point lists in the email body.\n"
        "- 'I' not 'we' — 'I wanted to share...' not 'We would like to inform...'\n"
        "- Mention specifics: machine models, cycle times, forming area, materials\n"
        "- NEVER say 'I hope this email finds you well', 'touching base', "
        "'reaching out', 'synergies', 'leverage', or corporate buzzwords\n"
        "- End casually: 'Let me know?', 'Worth a quick call?', 'Happy to chat.'\n"
        "- Every email teaches the prospect something they didn't know\n"
        "- Write 8-12 sentences for detailed emails, 4-6 for quick follow-ups\n"
    )

    REGIONAL_TONES = {
        "Germany": "Be precise and technical. Lead with specs and engineering credibility. Mention CE certification. Reference nearby European installations. Formal address.",
        "Austria": "Precise, technical, formal. Reference nearby German installations.",
        "Netherlands": "Direct and pragmatic. Value-for-money angle. Reference flagship European deliveries.",
        "Sweden": "Warm but professional. Oldest European market since 2001. Reference Nordic installations. Sustainability angle.",
        "UK": "Professional, slightly warm. Reference OEM partnerships and UK installations.",
        "France": "Polite and relationship-oriented. Reference French installations. Mention technology partnerships.",
        "Italy": "Warm and relationship-focused. Reference Italian installations. Mention 40+ year family heritage.",
        "India": "Warm, direct, price-conscious. Lead with ROI and payback period. Reference major Indian customers. Stock availability.",
        "UAE": "Relationship-first, respectful. Reference Gulf installations. KSA Vision 2030 housing boom for sanitaryware.",
        "Japan": "Extremely polite and precise. Reference Japanese customers. Emphasize servo precision and quality control.",
    }
    ...
    STAGE_DIRECTIVES = {
        ...
        4: "STAGE 4 — SOCIAL PROOF: Customer story from relevant region. Offer reference customer connection. Mention 35+ countries. CTA: 'Would you like to speak with [reference]?' 4-5 sentences.",
...
class ContextDossier:
    ...
    def to_prompt_context(self) -> str:
        ...
        # Board Meeting research — rich context from Tavily, CRM, Alexandros
        if self.past_interactions:
            parts.append(f"\nPAST INTERACTIONS (from CRM):\n{self.past_interactions}")
        if self.past_documents:
            parts.append(f"\nPAST DOCUMENTS (quotes, proposals found in archive):\n{self.past_documents}")
        if self.company_news:
            parts.append(f"\nCOMPANY NEWS (from web search):\n{self.company_news}")

        if self.news_hook:
            parts.append(f"\nNEWS HOOK (use this to open the email): {self.news_hook}")
        if self.personal_hook:
            parts.append(f"PERSONAL HOOK (reference to past interaction): {self.personal_hook}")
        ...
        if self.proof_stories:
            parts.append(f"\nPROOF STORIES (use the most relevant one):\n{self.proof_stories}")
        elif self.reference_story:
...
class ContextAssembler:
    ...
    async def assemble(self, lead_id: str) -> ContextDossier:
        ...
        # Board Meeting: deep research replaces broken Iris with working
        # Tavily + CRM history + archive search + LLM synthesis
        await self._run_board_meeting(dossier, campaign_profile)

        return dossier

    async def _run_board_meeting(self, dossier: ContextDossier, profile: Dict):
        """Run the Board Meeting research pipeline and merge results into dossier."""
        try:
            ...
            lead = {
                ...
                "notes": dossier.conversation_summary or profile.get("notes", ""),
                "name": dossier.contact_name,
                "source": profile.get("source", ""),
            }
            intel = await researcher.research(lead)

            if intel.get("company_news"):
                dossier.company_news = intel["company_news"]
            if intel.get("past_interactions"):
                dossier.past_interactions = intel["past_interactions"]
            if intel.get("past_documents"):
                dossier.past_documents = intel["past_documents"]
            if intel.get("proof_stories"):
                dossier.proof_stories = intel["proof_stories"]
            if intel.get("news_hook") and not dossier.news_hook:
                dossier.news_hook = intel["news_hook"]
            if intel.get("personal_hook"):
                dossier.personal_hook = intel["personal_hook"]
            if intel.get("company_insight") and not dossier.company_insight:
                dossier.company_insight = intel["company_insight"]
            if intel.get("subject_line"):
                dossier.industry_hook = dossier.industry_hook or intel.get("subject_line", "")

        except Exception as e:
            logger.warning("Board meeting research failed for %s: %s", dossier.company, e)
    ...
    def _fill_crm_context(self, dossier: ContextDossier, company: str):
        ...
        try:
            ...
            leads = self.crm.get_leads_by_company(company)
            if leads:
                lead = leads[0]
                dossier.emails_sent = lead.emails_sent or 0
                dossier.emails_received = lead.emails_received or 0
                dossier.deal_stage = lead.deal_stage or ""
                dossier.last_contact = lead.last_email_sent or lead.last_reply_at or ""
        except Exception as e:
    ...
    def _fill_reference_stories_dynamic(self, dossier: ContextDossier, country: str):
        stories_by_region = {
            "netherlands": ("[European Customer A] replaced their 40-year-old competitor machine with our PF1-X-1210. [European Customer B] ordered our biggest machine ever.", ["[European Customer A]", "[European Customer B]"]),
            "germany": ("[European Customer B] in the Netherlands ordered our biggest machine ever. [European Customer A] replaced a competitor machine. CE-certified, technology partners.", ["[European Customer B]", "[European Customer A]", "Technology partnership"]),
            "sweden": ("Sweden is our oldest European market since 2001. Multiple Swedish companies trust us across well covers, medical, and industrial applications.", ["[Nordic Customer A]", "[Nordic Customer B]"]),
            "uk": ("Our UK OEM partner since 2008. Multiple UK installations running.", ["[UK Partner]", "[UK Customer]"]),
            "italy": ("[Italian Customer] ordered a PF1-X-0707 — compact all-servo precision.", ["[Italian Customer]"]),
            "india": ("[Indian Customer A] came back for a second machine. [Indian Customer B] ordered a large PF1-X. [Indian Customer C] runs multiple machines for automotive.", ["[Indian Customer A]", "[Indian Customer B]", "[Indian Customer C]"]),
            "uae": ("[Gulf Customer] runs a Machinecraft machine. KSA Vision 2030 is driving sanitaryware demand.", ["[Gulf Customer]"]),
            "japan": ("[Japanese Customer A] and [Japanese Customer B] are active customers. Servo precision for automotive applications.", ["[Japanese Customer A]", "[Japanese Customer B]"]),
            "france": ("[French Customer] runs our machine. Technology partnership adds credibility in France.", ["[French Customer]"]),
            "denmark": ("[Danish Customer] — our newest Nordic installation.", ["[Danish Customer]"]),
        }
        ...
        dossier.reference_story = "Machines in 40+ countries. Recent flagship installations across Europe, India, and Asia."
        dossier.reference_customers = ["[European Customer]", "[Indian Customer]", "[Asian Customer]"]
    ...
    # [#2] Qdrant deep knowledge retrieval (falls back to Alexandros archive)
    def _fill_deep_knowledge(self, dossier: ContextDossier, company: str, profile: Dict):
        if not QDRANT_AVAILABLE:
            self._fill_deep_knowledge_from_archive(dossier, company, profile)
            return
        industries = profile.get("industries", [])
        query_parts = [company]
        if industries:
        ...
        try:
            ...
            if results:
                snippets = []
                citations = getattr(results, "citations", None) or results
                for r in (citations[:3] if hasattr(citations, '__getitem__') else []):
        ...
        if not dossier.deep_knowledge:
            self._fill_deep_knowledge_from_archive(dossier, company, profile)

    def _fill_deep_knowledge_from_archive(self, dossier: ContextDossier, company: str, profile: Dict):
        """Fallback: ask Alexandros for company/industry knowledge from the archive."""
        try:
            from openclaw.agents.ira.src.agents.alexandros import get_alexandros
        except ImportError:
            return
        try:
            alex = get_alexandros()
            industries = profile.get("industries", [])
            query = f"{company} {' '.join(industries[:2])} thermoforming vacuum forming"
            candidates = alex._fallback_fn(query, limit=2) if alex._fallback_fn else []
            if candidates:
                snippets = []
                for c in candidates:
                    path = c.get("path", "")
                    if path and alex._extract_fn:
                        text = alex._extract_fn(path)
                        if text and len(text.strip()) > 30:
                            snippets.append(text[:300].strip())
                if snippets:
                    dossier.deep_knowledge = "\n---\n".join(snippets)
                    logger.info(f"Hermes: Alexandros provided deep knowledge for {company}")
        except Exception as e:
            logger.debug(f"Alexandros archive fallback skipped: {e}")
...
class EmailCrafter:
    ...
    async def craft(dossier: ContextDossier, ab_test: bool = True) -> Dict[str, Any]:
        ...
        chiron_notes = ""
        try:
            from openclaw.agents.ira.src.agents.chiron import get_coaching_notes
            situation = f"stage {stage} email, company: {dossier.company}, region: {dossier.country}"
            if hasattr(dossier, 'crm_summary') and dossier.crm_summary:
                situation += f", context: {dossier.crm_summary[:200]}"
            chiron_notes = get_coaching_notes(situation)
        except Exception:
            pass

        chiron_block = f"{chiron_notes}\n" if chiron_notes else ""

        board_meeting_rules = (
            "BOARD MEETING RULES (critical — these make emails good):\n"
            "- You MUST use the NEWS HOOK in your opening line. Never open with a generic greeting.\n"
            "- If PAST INTERACTIONS exist, reference them specifically — names, dates, what was discussed.\n"
            "- If PAST DOCUMENTS exist (quotes, proposals), mention them by name and ask if they're still relevant.\n"
            "- If a PERSONAL HOOK is provided, weave it into the email naturally.\n"
            "- Use the most relevant PROOF STORY — include specific numbers (forming area, cycle time, material).\n"
            "- Write 8-15 sentences for a detailed, substantive email. NOT a 4-sentence template.\n"
            "- The email should feel like it was written by someone who spent 30 minutes researching this company.\n"
            "- PF1 series does VACUUM FORMING only. Never mention pressure forming for PF1.\n"
        )
        ...
        system_prompt = (
            ...
            f"{stage_directive}\n\n"
            f"{board_meeting_rules}\n"
            f"{guardrails}\n"
            f"{ab_instruction}"
            f"{chiron_block}"
            "Sign off as: Rushabh Doshi | Sales Head, Machinecraft Technologies | rushabh@machinecraft.in\n"
            "Return valid JSON.\n"
        )
    ...
    def _fallback_email(dossier: ContextDossier) -> Dict[str, Any]:
        ...
        return {
            ...
            "body": (
                ...
                f"I'm Rushabh from Machinecraft — we build thermoforming "
                f"machines, installed in 35+ countries.\n\n"
                f"{'I noticed ' + dossier.news_hook + '. ' if dossier.news_hook else ''}"
                f"I think our machines could be a good fit for your operation"
                f"{' — the ' + dossier.recommended_machine + ' in particular' if dossier.recommended_machine else ''}.\n\n"
                f"Worth a quick look?\n\n"
                f"Best,\nRushabh Doshi\nSales Head, Machinecraft Technologies\n"
                f"rushabh@machinecraft.in\nwww.machinecraft.in"
            ),
            "stage": dossier.next_stage,
