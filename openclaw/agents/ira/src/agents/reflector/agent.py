...
async def reflect(interaction_data: Dict[str, Any]) -> ReflectionResult:
    ...
    # Evaluate quality — prefer LLM scoring for substantive responses
    quality = None
    if len(response) > 100:
        quality = _evaluate_quality_llm(user_message, response, intent)
    if quality is None:
...
def _evaluate_quality_llm(user_message: str, response: str, intent: str) -> Optional[QualityScore]:
    """LLM-based quality evaluation — more accurate than heuristics for substantive responses."""
    try:
        import openai
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    '{"factual_accuracy": 0.8, "helpfulness": 0.7, "completeness": 0.9, '
                    '"tone": 0.8, "structure": 0.7, "responsiveness": 0.8}'
                )},
        ...
        logger.debug(f"LLM quality evaluation failed, falling back to heuristic: {e}")
        return None


def _evaluate_quality(user_message: str, response: str, intent: str) -> QualityScore:
    """Heuristic quality evaluation — fast fallback when LLM is unavailable."""
    score = QualityScore()
...
def _identify_issues(user_message: str, response: str, results: Dict) -> List[str]:
    ...
    # Check for empty or very short response (context-aware)
    if len(response) < 50:
        msg_lower = user_message.lower().strip()
        is_greeting = any(g in msg_lower for g in [
            "hi", "hello", "hey", "good morning", "good evening", "good afternoon",
        ])
        is_ack = any(a in msg_lower for a in [
            "thanks", "thank you", "ok", "got it", "cool", "great", "nice",
        ])
