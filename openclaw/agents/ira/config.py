...
# PostgreSQL - Canonical connection string (MUST be set in .env)
...
RUSHABH_TELEGRAM_ID = os.environ.get("RUSHABH_TELEGRAM_ID", "") or EXPECTED_CHAT_ID
...
def multi_storage_write(
    ...
    if json_path and json_path.exists():
        try:
            ...
            logger.error(f"Error in multi_storage_write: {e}", exc_info=True)
    ...
    try:
        ...
        if f:
            try:
                ...
                logger.error(f"Error in locked_file: {e}", exc_info=True)
            f.close()
