...
Total: {len(draft_ids)}""",
    ...
    def route_message(self, message: TelegramMessage) -> GatewayResponse:
        ...
        # URL ingestion — detect a URL (optionally with context after it)
        url_match = re.match(r'^(https?://\S+)\s*(.*)', text, re.IGNORECASE)
        if url_match:
            url = url_match.group(1).rstrip(".,;:!?)")
            context = url_match.group(2).strip() or None
            return self.handle_url_ingest(url, context)
