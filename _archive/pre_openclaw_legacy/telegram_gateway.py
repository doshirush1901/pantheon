...
# ---------------------------------------------------------------------------
# HTML report generation for Telegram document attachments
# ---------------------------------------------------------------------------

def _md_to_html_report(markdown_text: str, query: str, elapsed: float) -> str:
    """Convert a markdown research report to a styled HTML document."""
    try:
        ...
        body_html = _md_fallback_convert(markdown_text)
...
<title>Ira Research Report</title>
<style>
  :root {{ --bg: #ffffff; --fg: #1a1a2e; --accent: #2b4b96; --muted: #6b7280;
           --border: #e5e7eb; --code-bg: #f3f4f6; --card-bg: #f9fafb; }}
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  h1, h2, h3 {{ margin-top: 1.5em; margin-bottom: 0.5em; color: var(--accent); }}
  ...
  blockquote {{ border-left: 3px solid var(--accent); padding-left: 12px; color: var(--muted);
                margin: 1em 0; }}
  .footer {{ margin-top: 32px; padding-top: 16px; border-top: 1px solid var(--border);
             color: var(--muted); font-size: 0.8em; text-align: center; }}
</style>
</head>
<body>
<div class="header">
  <h1>Ira Research Report</h1>
  <div class="meta">
    <span>{date_str}</span>
    <span>{elapsed:.0f}s research</span>
  </div>
  <p style="margin-top:8px; color: var(--muted); font-size:0.9em;"><em>{query}</em></p>
</div>
{body_html}
<div class="footer">
  Generated by Ira &mdash; Machinecraft Technologies
</div>
</body>
</html>"""


def _md_fallback_convert(text: str) -> str:
    """Minimal markdown-to-HTML when the markdown library is unavailable."""
        ...
        if UNIFIED_AVAILABLE:
            ...
            try:
                ...
                if _agent_response and len(_agent_response.strip()) > 10:
                    ...
                    self._pending_full_report = _agent_context.get("_full_report")
...
Total: {len(draft_ids)}""",
    ...
    _TOOL_PHASE_MAP = {
        "research_skill": "gathering", "research": "gathering",
        "memory_search": "gathering", "memory": "gathering",
        "customer_lookup": "gathering", "crm_list_customers": "gathering",
        "crm_pipeline": "gathering", "web_search": "gathering",
        "lead_intelligence": "gathering", "iris": "gathering",
        "search_email": "gathering", "read_email_thread": "gathering",
        "read_inbox": "gathering", "search_drive": "gathering",
        "read_spreadsheet": "gathering", "check_calendar": "gathering",
        "finance_overview": "gathering", "order_book_status": "gathering",
        "cashflow_forecast": "gathering", "revenue_history": "gathering",
        "discovery_scan": "gathering",
        "run_analysis": "analysis",
        "writing_skill": "synthesis", "writing": "synthesis",
        "draft_email": "synthesis", "build_quote_pdf": "synthesis",
        "fact_checking_skill": "verification", "verifying": "verification",
        "vera_verify": "verification",
        "sophia_reflect": "polish", "polishing": "polish",
        "ask_user": "gathering",
    }

    _PHASE_ORDER = ["gathering", "analysis", "synthesis", "verification", "polish"]
    _PHASE_TITLES = {
        "gathering":    "Gathering",
        "analysis":     "Analysis",
        "synthesis":    "Synthesis",
    ...
    def _update_thinking_status(self, msg_id: int, stage: str, elapsed: float):
        """Update the thinking indicator with Manus-style phased parallel view."""
        # Handle phase transition signals from the orchestrator
        if stage.startswith("phase:"):
            self._current_phase = stage.split(":", 1)[1]
            # Don't add phase signals as tool steps, just update the display
        else:
            label = self._TOOL_LABELS.get(stage, stage.replace("_", " ").title())
            tool_phase = self._TOOL_PHASE_MAP.get(stage, self._current_phase)
            if stage not in [s for s, _, _ in self._thinking_steps]:
                self._thinking_steps.append((stage, label, tool_phase))

        # Build phase-grouped display
        total_tools = len(self._thinking_steps)
        active_tools = sum(
            1 for s, _, _ in self._thinking_steps
            if s == stage or (not stage.startswith("phase:") and s == stage)
        )
        # Count tools still in the current phase as "active"
        active_count = sum(
            1 for s, _, p in self._thinking_steps
            if p == self._current_phase
        )

        depth_label = "🔬 Deep Research" if total_tools >= 6 else "🧠 Researching"
        header = f"*{depth_label}* ({total_tools} agents"
        if active_count > 1:
        ...
        current_phase_idx = (
            ...
            if self._current_phase in self._PHASE_ORDER else 0
        )

        for phase_idx, phase_key in enumerate(self._PHASE_ORDER):
            phase_tools = [
                (s, lbl) for s, lbl, p in self._thinking_steps if p == phase_key
            ]

            if phase_idx > current_phase_idx and not phase_tools:
                lines.append(f"⋯ *Phase {phase_idx + 1}: {self._PHASE_TITLES[phase_key]}* _(pending)_")
                continue

            if not phase_tools and phase_idx < current_phase_idx:
                continue

            if phase_tools or phase_key == self._current_phase:
                lines.append(f"\n*Phase {phase_idx + 1}: {self._PHASE_TITLES[phase_key]}*")
                for s, lbl in phase_tools:
                    if phase_key == self._current_phase and s == stage:
                        lines.append(f"  ▸ {lbl}...")
                    else:
                        lines.append(f"  ✓ {lbl}")
    ...
    def process_message(self, message: TelegramMessage) -> None:
        ...
        self._current_phase = "gathering"
        self._pending_quote_files = []
        self._pending_full_report = None
        ...
        if response.text:
            ...
            # Determine full report content: if the orchestrator generated a
            # summary, the full report is stashed in _pending_full_report;
            # otherwise fall back to response.text for long responses.
            full_report_text = getattr(self, "_pending_full_report", None)
            _REPORT_THRESHOLD = 2000

            report_content = full_report_text or response.text
            if len(report_content) > _REPORT_THRESHOLD and not is_command:
                try:
                    reports_dir = Path("data/exports/reports")
                    reports_dir.mkdir(parents=True, exist_ok=True)
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    slug = re.sub(r"[^a-z0-9]+", "_", message.text[:40].lower()).strip("_")
                    report_file = reports_dir / f"ira_report_{ts}_{slug}.html"

                    html_content = _md_to_html_report(
                        report_content, message.text, elapsed,
                    )
                    report_file.write_text(html_content, encoding="utf-8")
                    logger.info("[Report] Saved %d-char HTML report to %s",
                                len(report_content), report_file)
                except Exception as e:
                    logger.warning("[Report] Failed to save HTML report: %s", e)
                    report_file = None
            ...
            if report_file and report_file.exists():
                ...
                self.send_document(
                    str(report_file),
                    caption=f"Full report ({char_count:,} chars, {elapsed:.0f}s research)",
                )
