...
CRITICAL RULE: Only reply to threads where RUSHABH sent the LAST message.
Ira waits for Rushabh to reply before sending another message.

Logic:
1. Check inbox for threads with recent activity
2. For each thread, check who sent the LAST message
3. Only reply if Rushabh (not Ira) sent the last message
...
def get_threads_needing_reply(hours: int = 1):
    """
    Get threads where RUSHABH sent the LAST message (Ira needs to reply).
    
    CRITICAL: 
    - Only return threads where Rushabh's message is the most recent
    ...
    # Get threads with recent activity from Rushabh
    after_date = datetime.now() - timedelta(hours=hours)
    query = f"after:{after_date.strftime('%Y/%m/%d')} from:rushabh@machinecraft.org"
    ...
    for msg_ref in messages:
        ...
        # CRITICAL: Only reply if RUSHABH sent the last message
        # Skip if Ira sent the last message (we're waiting for Rushabh)
        if 'ira@' in from_email or 'ira@machinecraft' in from_email:
            print(f"  [SKIP] Thread '{headers.get('subject', '?')[:40]}' - Ira sent last, waiting for Rushabh")
            continue
        ...
        # This is a thread where Rushabh sent the last message - we need to reply!
        # Parse the message
...
Do NOT include greeting or signature - those will be added separately."""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Reply to Rushabh:\n\n{email['body']}"}
        ],
        max_tokens=400,
        temperature=0.7,
    )
    ...
    if POLISH_AVAILABLE:
        ...
        # Step 1: Polish with Ira's personality + Rushabh's style
        polish_result = polisher.polish(
            draft_email=raw_reply,
            recipient_style={
                "formality_score": 35,  # Rushabh is casual
                "detail_score": 70,
                "technical_score": 80,
                "humor_score": 70,
            },
            emotional_state="neutral",
            warmth="trusted",  # Rushabh is boss/trusted
            use_llm=True,
        )
        ...
        formatted = styler.format_email_response(
            ...
            add_offer_help=False,  # Not needed for internal replies
        )
...
╔════════════════════════════════════════════════════════════════════╗
║                     IRA REPLY NOW (SMART)                          ║
║  Only replies when RUSHABH sent the last message                   ║
║  Waits for Rushabh's reply before sending again                    ║
╚════════════════════════════════════════════════════════════════════╝
""")
    ...
    print(f"Found {len(threads)} thread(s) where Rushabh is waiting for Ira's reply\n")
    ...
    for i, thread in enumerate(threads, 1):
        ...
        print(f"Rushabh said: {thread['body'][:100]}...")
        print(f"{'='*60}")
        ...
        if not args.dry_run:
            ...
            success = send_email_gmail(
                ...
                subject=subject,
                body=reply,
                from_email="ira@machinecraft.in",
            )
