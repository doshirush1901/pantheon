...
- Warm and genuinely helpful with Rushabh (your colleague/boss)
...
def send_reply(reply: str, thread_id: str, to_email: str, subject: str):
    ...
    message['to'] = to_email
    message['from'] = 'ira@machinecraft.org'
...
def main():
    """Process the most recent unread email from Rushabh."""
    print("\n" + "=" * 60)
    ...
    results = service.users().messages().list(
        userId='me',
        q='from:rushabh@machinecraft.org newer_than:2h is:unread',
        maxResults=1
    ).execute()
    
    messages = results.get('messages', [])
    if not messages:
        print("No unread messages from Rushabh.")
        return
    ...
    sent = send_reply(
        reply,
        thread_id,
        headers.get('From', 'rushabh@machinecraft.org'),
