...
# Tool orchestrator — the same agentic pipeline used by Telegram, without
# instantiating TelegramGateway (which would conflict with the live bot).
ORCHESTRATOR_AVAILABLE = False
try:
    from openclaw.agents.ira.src.core.tool_orchestrator import process_with_tools
    ORCHESTRATOR_AVAILABLE = True
    logger.info("Tool orchestrator loaded (v22 agentic pipeline)")
except ImportError as e:
    logger.warning(f"Tool orchestrator not available: {e}")
...
# Patterns that indicate an address Ira should NEVER reply to.
...
_SPAM_SENDER_DOMAINS = {
    ...
    "constantcontact.com", "hubspot.com", "mailgun.org", "mailgun.net",
    "sendinblue.com", "brevo.com", "klaviyo.com", "campaign-archive.com",
    "nordiskemedier.dk", "coach.com",
}

_SPAM_HEADER_KEYWORDS = re.compile(
    r"(?i)"
    r"unsubscribe|list-unsubscribe|precedence:\s*(bulk|list|junk)"
    r"|x-mailer:.*(mailchimp|sendgrid|hubspot|campaign)"
    r"|feedback-id:"
)

LOCK_FILE = PROJECT_ROOT / "data" / ".email_bridge.lock"


def _should_skip_sender(email_data: Dict) -> Optional[str]:
    ...
    if _NOREPLY_PATTERNS.search(addr):
        return f"noreply/automated address: {addr}"

    domain = addr.rsplit("@", 1)[-1]
    # Walk up subdomains: newsletter.linkedin.com -> linkedin.com
    parts = domain.split(".")
    for i in range(len(parts) - 1):
        check = ".".join(parts[i:])
        if check in _SPAM_SENDER_DOMAINS:
            return f"blocked domain: {check}"

    subject = (email_data.get("subject") or "").lower()
    body_snippet = (email_data.get("snippet") or email_data.get("body") or "")[:500].lower()
    combined = f"{subject} {body_snippet}"
    if _SPAM_HEADER_KEYWORDS.search(combined):
        return f"newsletter/marketing content detected"

    # Catch generic marketing patterns in subject
    marketing_subjects = [
        "unsubscribe", "your weekly", "your daily", "your monthly",
        "newsletter", "digest", "% off", "% discount", "limited time",
        "don't miss", "exclusive offer", "act now", "sale ends",
    ]
    for pattern in marketing_subjects:
        if pattern in subject:
            return f"marketing subject line: '{pattern}'"
...
def _acquire_lock() -> bool:
    ...
    try:
        ...
        lock_fd.flush()
        # Keep fd open for lifetime of process (lock released on exit)
...
class IraAgentBridge:
    ...
    6. CREATES FOLLOW-UP QUESTIONS for the founder
    ...
    def process_email(self, body: str, from_email: str, subject: str, thread_id: str) -> Optional[str]:
        ...
        6. Question Generation - Create follow-up questions for the founder
        ...
        try:
            ...
            if self.deep_research_available and self.deep_research:
                ...
                # Log follow-up questions for the founder
...
class EmailIraBridge:
    ...
    def __init__(self):
        ...
        if not IRA_AGENT_AVAILABLE and not ORCHESTRATOR_AVAILABLE:
            raise RuntimeError("Neither IRA Agent nor Tool Orchestrator available")

        self.gmail = GmailClient()
        self.ira = IraAgentBridge() if IRA_AGENT_AVAILABLE else None
                    ...
                    rewrite_result = openai_client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": (
                                ...
                                "- If the sender is the founder (internal), this is a strategy request — be direct.\n"
                ...
                package_result = openai_client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": (
                            ...
                            "- Start with 'Hi Founder,' if internal, or 'Hi [Name],' if customer\n"
    ...
    def process_single_email(self, email: Dict) -> bool:
        ...
        skip_reason = _should_skip_sender(email)
        if skip_reason:
        ...
        if is_feedback:
            ...
            confirmation = self.ira.store_feedback(from_email, body, original_response) if self.ira else None
        ...
        # Use the agentic tool orchestrator directly (same pipeline as Telegram)
        response = None
        if ORCHESTRATOR_AVAILABLE:
            try:
                import asyncio
                logger.info("Using tool orchestrator (v22 agentic pipeline) for email processing")

                _conv_history = ""
                if thread_history:
                    for msg in thread_history[-6:]:
                        role = msg.get('role', 'user').upper()
                        content = msg.get('body', '')[:400]
                        _conv_history += f"{role}: {content}\n"

                def _email_progress(event):
                    if isinstance(event, dict):
                        agent = event.get("agent", "")
                        activity = event.get("activity", "")
                        logger.info("[Pipeline] %s — %s", agent, activity)
                ...
                _context = {
                    ...
                    "_progress_callback": _email_progress,
                }
                ...
                if _result and isinstance(_result, str) and len(_result.strip()) > 20:
                    response = _result
                    logger.info(f"Tool orchestrator produced {len(response)} chars")
                else:
                    logger.warning("Tool orchestrator returned empty, falling back to IraAgent")
            except Exception as e:
                logger.warning(f"Tool orchestrator failed, falling back to IraAgent: {e}")
                import traceback
                logger.debug(traceback.format_exc())

        # FALLBACK: Use IraAgent if gateway engine unavailable or failed
        if not response and self.ira:
            ...
            if success:
                ...
                try:
                    ...
                    inbound_items = extract_email_knowledge(
                        ...
                        to_email=os.environ.get("IRA_EMAIL", "ira@example-company.org"),
                    ...
                    outbound_items = extract_email_knowledge(
                        subject=f"Re: {subject}", body=response, from_email="ira@example-company.org",
    ...
    def _send_proactive_email(
        ...
        """Send a proactive email to the founder. Uses different research prompts and templates per type."""
        import asyncio
        ...
        templates = {
            "startup": (
                "You are Ira, the Intelligent Revenue Assistant for Machinecraft Technologies. "
                "Write a brief, warm check-in email to the founder (your boss). Include: "
            ...
            "daily": (
                "You are Ira, the Intelligent Revenue Assistant for Machinecraft Technologies. "
                "Write a short daily digest email to the founder. Start with 'Your daily digest:' "
            ...
            "weekly": (
                "You are Ira, the Intelligent Revenue Assistant for Machinecraft Technologies. "
                "Write a longer weekly briefing email to the founder. Structure: "
    ...
    def send_proactive_checkin(self):
        """Send a proactive check-in email to the founder on startup (I'm online + top 2-3 pending)."""
        logger.info("Sending proactive check-in to the founder...")
        try:
    ...
    def run_loop(self):
        ...
        if self.ira:
            ...
            logger.info("Running with Gateway Engine (IraAgent not available)")
...
def main():
    ...
    if args.loop and not _acquire_lock():
        logger.error("Another email bridge instance is already running. Exiting.")
        sys.exit(1)
